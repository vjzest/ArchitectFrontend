...
const InquiryDetailModal = ({ inquiryId, onClose, onUpdate }) => {
  ...
  useEffect(() => {
    if (selectedInquiry) setStatus(selectedInquiry.status);
  }, [selectedInquiry]);
  ...
  const handleStatusUpdate = () => {
    dispatch(updateInquiryStatus({ inquiryId, status })).then((result) => {
      if (result.meta.requestStatus === "fulfilled") {
        ...
        onUpdate(); // List ko refresh karne ke liye
        onClose();
      } else {
  ...
  if (!selectedInquiry && actionStatus === "loading")
    return (
      ...
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-8">
          ...
          <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
            ...
            <div className="bg-gray-50 p-4 rounded-lg space-y-2">
              <h3 className="font-bold text-gray-700 flex items-center gap-2">
                <ProductIcon size={16} /> Product Info
              </h3>
              <p>
            ...
            <div className="md:col-span-2 bg-gray-50 p-4 rounded-lg">
              <h3 className="font-bold text-gray-700 mb-2">Message</h3>
...
// --- SELLER INQUIRIES PAGE COMPONENT ---
const SellerInquiriesPage = () => {
  const dispatch: AppDispatch = useDispatch();
  const { inquiries, listStatus, error } = useSelector(
    (state: RootState) => state.sellerInquiries
  );
